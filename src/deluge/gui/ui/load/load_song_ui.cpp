...
void LoadSongUI::enterKeyPress() {
	...
	if (currentFileItem && currentFileItem->isFolder) {
		...
		// Load-guard: don't let an accidental load (e.g. a double-LOAD) wipe unsaved work. First LOAD
		// over an unsaved song just arms + warns; you press LOAD again to actually load. Covers a
		// never-saved jam AND an edited existing song (same dirty flag). Navigating away or BACK
		// resets the arm (see currentFileChanged/exitAction) — and it never forces a save: a second
		// LOAD simply discards the unwanted song. performLoad stays on its normal lifecycle below.
		...
		// clear the recovery file. This is what keeps "RECOVER exists ⟺ unsaved work" true on load-away
		// (without it, you'd get re-recovered every boot). The new song re-creates RECOVER once edited.
...
void LoadSongUI::currentFileChanged(int32_t movementDirection) {
	...
	// Selection moved → must re-confirm a load-over-unsaved (can't load the wrong song on a stale arm).
	confirmedLoadOverUnsaved = false;
