...
// Autosave/recovery (slice 1): a structural change sets this; a low-priority card-safe task
// writes the recovery file when transport is stopped and the card is free. See writeRecoveryFile().
extern bool songNeedsRecoverySave;

namespace StorageManager {

std::expected<FatFS::File, Error> createFile(char const* filePath, bool mayOverwrite);
// Serialize the current song to SYSTEM/RECOVER.XML (temp + atomic rename). UI-free; does not
// touch the user's named-save state. Caller must ensure the card is free (not currentlyAccessingCard).
Error writeRecoveryFile();
// Delete the recovery file + clear the dirty flag. Call after a manual Save / on load-away — this
// is what makes "RECOVER exists ⟺ unsaved work" hold without a clean-shutdown signal.
